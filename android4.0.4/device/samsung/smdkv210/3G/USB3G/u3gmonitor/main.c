...
//Apollo +
      SLOGI("3g_model usb_vid=%s usb_pid=%s\n", usb_vid, usb_pid); 			
      if((!strcmp("05c6", usb_vid)) && (!strcmp("6000", usb_pid)))
        system("/sbin/busybox cp -rf /system/etc/ppp/call-pppd_evdo /system/etc/ppp/call-pppd");
      else
        system("/sbin/busybox cp -rf /system/etc/ppp/call-pppd_cdma /system/etc/ppp/call-pppd");
      
      system("chmod 0777 /system/etc/ppp/call-pppd");
      sync();
//Apollo -
