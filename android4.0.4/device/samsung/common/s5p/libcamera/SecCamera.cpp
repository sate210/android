...
*/
void startflash(int fd)
{
      if (fimc_v4l2_s_ctrl(fd, V4L2_CID_CAMERA_START_FLASH, 0) < 0) {
                                            ...
                                            unsigned int *output_size)
{
    ...
    unsigned char *addr;
    int ret = 0;
		unsigned char *yuv_tmp;
		unsigned char *start_tmp;
		unsigned int loop = 0;
		unsigned int size = 0;
...
//  loop = 4;
//  size  = m_snapshot_width * m_snapshot_height * 2/loop;
//  yuv_tmp = yuv_buf;
//  start_tmp = (unsigned char*)m_capture_buf.start;
//  while(loop--){
//    memcpy(yuv_tmp, start_tmp, size);
//    yuv_tmp += size;
//   	start_tmp += size;
//   	usleep(50000);
//   	LOGI("loop = %d +++++++++++++++++++++++++++++++++",loop);
...
//		{
//			unsigned long i, len;
//			len = m_snapshot_width * m_snapshot_height * 2;
//			for(i=0; i<len; i++)
//			{
//				//*(((unsigned char *)yuv_tmp)++) = *(((unsigned char *)start_tmp)++);
//				*yuv_tmp++ = *start_tmp++;
//			}
//		}
