          ...
          mHalDevice(dev)
{
    ...
    LOGE("mPostViewWidth = %d mPostViewHeight = %d mPostViewSize = %d",
...
void CameraHardwareSec::initDefaultParameters(int cameraId)
{
    ...
    LOGE("CameraSensorName: %s", mCameraSensorName);
...
status_t CameraHardwareSec::setPreviewWindow(preview_stream_ops *w)
{
    ...
    mPreviewWindow = w;
    LOGE("%s: mPreviewWindow %p", __func__, mPreviewWindow);
    ...
    LOGE("%s: setting buffer count to %d", __func__, kBufferCount);
    ...
    LOGE("%s: preview format %s", __func__, str_preview_format);
...
{
    LOGE("%s : msgType = 0x%x, mMsgEnabled before = 0x%x",
         __func__, msgType, mMsgEnabled);
    mMsgEnabled |= msgType;

    LOGE("%s : mMsgEnabled = 0x%x", __func__, mMsgEnabled);
}
...
//  LOGE("%s: index %d", __func__, index);

    mSkipFrameLock.lock();
    if (mSkipFrame > 0) {
        mSkipFrame--;
        mSkipFrameLock.unlock();
        LOGE("%s: index %d skipping frame", __func__, index);
        return NO_ERROR;
    }
    mSkipFrameLock.unlock();
...
status_t CameraHardwareSec::startPreviewInternal()
{
    ...
    LOGE("%s : mSecCamera->startPreview() returned %d", __func__, ret);
    ...
    mSecCamera->getPostViewConfig(&mPostViewWidth, &mPostViewHeight, &mPostViewSize);
    LOGE("CameraHardwareSec: mPostViewWidth = %d mPostViewHeight = %d mPostViewSize = %d",
         mPostViewWidth,mPostViewHeight,mPostViewSize);
...
bool CameraHardwareSec::previewEnabled()
{
    ...
    LOGE("%s : %d", __func__, mPreviewRunning);
...
int CameraHardwareSec::autoFocusThread()
{
    ...
    if (mExitAutoFocusThread) {
        mFocusLock.unlock();
        LOGE("%s : exiting on request0", __func__);
        return NO_ERROR;
    }
    ...
    if (mExitAutoFocusThread) {
        mFocusLock.unlock();
        LOGE("%s : exiting on request1", __func__);
        return NO_ERROR;
    }
    mFocusLock.unlock();

#ifdef AF_SUPPORT
    LOGE("%s : calling setAutoFocus", __func__);
    ...
    if (af_status == 0x01) {
        LOGE("%s : AF Success!!", __func__);
        if (mMsgEnabled & CAMERA_MSG_FOCUS)
            mNotifyCb(CAMERA_MSG_FOCUS, true, 0, mCallbackCookie);
    } else if (af_status == 0x02) {
        LOGE("%s : AF Cancelled !!", __func__);
        if (mMsgEnabled & CAMERA_MSG_FOCUS) {
        ...
        LOGE("%s : AF Fail !!", __func__);
        LOGE("%s : mMsgEnabled = 0x%x", __func__, mMsgEnabled);
        if (mMsgEnabled & CAMERA_MSG_FOCUS)
            mNotifyCb(CAMERA_MSG_FOCUS, false, 0, mCallbackCookie);
    }

    LOGE("%s : exiting with no error", __func__);
    return NO_ERROR;
}
...
//    sp<MemoryBase> buffer = new MemoryBase(mRawHeap, 0, mPostViewSize + 8);
    ...
    LOGE("[5B] mPostViewWidth = %d mPostViewHeight = %d\n",mPostViewWidth,mPostViewHeight);
    ...
    LOGE("%s : pictureThread end", __func__);
...
status_t CameraHardwareSec::takePicture()
{
    ...
    if (!mRawHeap) {
        ...
        LOGE("mRawHeap : MemoryHeapBase(previewHeapSize(%d))", rawHeapSize);
...
status_t CameraHardwareSec::cancelPicture()
{
    ...
    if (mPictureThread.get()) {
        LOGE("%s: waiting for picture thread to exit", __func__);
        mPictureThread->requestExitAndWait();
        LOGE("%s: picture thread has exited", __func__);
    }
...
status_t CameraHardwareSec::setParameters(const CameraParameters& params)
{
    ...
    LOGE("%s : new_preview_width x new_preview_height = %dx%d, format = %s",
    ...
    if (0 < new_preview_width && 0 < new_preview_height &&
        ...
        if (current_preview_width != new_preview_width ||
            ...
            if (mSecCamera->setPreviewSize(new_preview_width, new_preview_height,
                ...
                if (mPreviewWindow) {
                    ...
                    LOGE("%s: mPreviewWindow (%p) set_buffers_geometry", __func__, mPreviewWindow);
                    LOGE("%s: mPreviewWindow->set_buffers_geometry (%p)", __func__,
                    ...
                    mPreviewWindow->set_buffers_geometry(mPreviewWindow,
                                                         ...
                                                         new_preview_format);
                    LOGE("%s: DONE mPreviewWindow (%p) set_buffers_geometry", __func__, mPreviewWindow);
                }
        ...
        else LOGE("%s: preview size and format has not changed", __func__);
    } else {
    ...
    LOGE("%s : new_picture_width x new_picture_height = %dx%d", __func__, new_picture_width, new_picture_height);
    if (0 < new_picture_width && 0 < new_picture_height) {
        LOGE("%s: setSnapshotSize", __func__);
    ...
    LOGE("%s : new_str_picture_format %s", __func__, new_str_picture_format);
    ...
    LOGE("%s : new_jpeg_quality %d", __func__, new_jpeg_quality);
    ...
    LOGE("%s : new_exposure_compensation %d", __func__, new_exposure_compensation);
    ...
    LOGE("%s : new_white_str %s", __func__, new_white_str);
